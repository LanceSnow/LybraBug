...
contract MockLybra is EUSD, Governable {
    ...
    function _etherPrice() internal view returns (uint256) {
        return IPriceFeed(_priceFeed).fetchPrice();
    }
    ...
    function collateralRateOf(
        ...
        uint256 onBehalfOfCollateralRate = (depositedEther[onBehalfOf] *
            etherPrice *
            100) / borrowed[onBehalfOf];
        return onBehalfOfCollateralRate;
    }

    function borrowedLimitOf(
        uint256 depositedEther_
    ) public view returns (uint256) {
        uint256 etherPrice = _etherPrice();
        uint256 limited = (depositedEther_ * etherPrice * 100) /
            safeCollateralRate;
        return limited;
    }
}
